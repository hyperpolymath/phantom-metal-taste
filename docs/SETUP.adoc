...
You should see both `+phantom-arangodb+` and `+phantom-virtuoso+` with
status `+healthy+`.

==== 6. Verify Database Connections

*ArangoDB:* - Open http://localhost:8529 - Login: `+root+` /
`+phantom-dev-password+` - Database `+phantom_metal_taste+` should exist

*Virtuoso:* - Open http://localhost:8890/conductor - Login: `+dba+` /
`+phantom-dev-password+` - SPARQL endpoint available at
...
This loads: - 4 departments - 6 sample employees - 3 initiatives
(Wellness Wednesday, Transparency Dashboard, Synergy Champions) - 9
outcomes (intended and unintended) - Causal links - Sample metrics
...
Make changes to ReScript files, rebuild, and the server will restart
automatically.
...
After loading SynapCor data, get initiative ID from web UI or query,
then:
...
[arabic]
. Explore the SynapCor case study: `+case-studies/synapcor/README.md+`
. Read architecture docs: `+docs/architecture/ARCHITECTURE.md+`
...
_"`Setting up a system to measure organizational delusion requires
meticulous attention to detail. The irony is not lost on us.`"_

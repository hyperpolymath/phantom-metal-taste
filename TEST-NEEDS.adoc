...
|Rust unit tests |4 |`+src/core/src/lib.rs+` `+#[cfg(test)]+` — all
passing
...
* [x] `+calculate_synergy_score+` — penalises suspiciously uniform high
scores
* [x] `+phantom_metal_taste_score+` — composite score from
outcome/gap/theater inputs
...
* `+wasm-bindgen+` dependency: tests compile via `+rlib+` but WASM
target untested
* `+calculate_metric_gaps+` test only checks string presence, not parsed
values
* `+detect_metric_anomalies+` and `+detect_metric_theater+` have no test
coverage
...
* [ ] Tests for `+detect_metric_anomalies+` and
`+detect_metric_theater+`
* [ ] Julia test suite for `+causal_inference.jl+` and
`+impact_analysis.jl+`
* [ ] ReScript/Deno tests for orchestrator layer
* [ ] Migrate `+benchmarks/gap_calculation.bench.ts+` from TypeScript to
Deno
